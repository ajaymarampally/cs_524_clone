...
app.get('/api/state_info', function(req, res) {
  ...
  const STATE_INFO = `select a.iso_region,a.airport_name, a.latitude, a.longitude,'dep' direction,ROUND(avg(dep_delay)::NUMERIC, 3) delay, ROUND(avg(taxi_out)::NUMERIC, 3) taxi, ROUND(sum(flight_count)::NUMERIC,3) total_count
  ...
  group by 1,2,3,4,5
  UNION
  select a.iso_region,a.airport_name,  a.latitude, a.longitude,'arr' direction, ROUND(avg(arr_delay)::NUMERIC, 3) delay, ROUND(avg(taxi_in)::NUMERIC, 3) taxi, ROUND(sum(flight_count)::NUMERIC,3) total_count
  from flight_summary fs, us_airports a
  where
  a.iso_region = $1
